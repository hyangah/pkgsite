...
func TestRunHelp(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "pkginfo") {
		t.Error("help output does not contain 'pkginfo'")
	}
}
...
func TestRunPackageJSON(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	code := run([]string{"--json", "--server=" + srv.URL, "encoding/json"}, &stdout, &stderr)
	if code != 0 {
		t.Fatalf("exit code = %d, stderr = %s", code, stderr.String())
	}
...
func TestRunAPIError(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	code := run([]string{"--server=" + srv.URL, "nonexistent/pkg"}, &stdout, &stderr)
	if code != 1 {
...
func TestRunAPIErrorJSON(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	code := run([]string{"--json", "--server=" + srv.URL, "nonexistent/pkg"}, &stdout, &stderr)
	if code != 1 {
		t.Errorf("exit code = %d, want 1", code)
	}
